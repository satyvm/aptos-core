...
// TODO: Specifying the same field twice in separate structs in the config is clearly
// bad form, fix it.
impl IndexerGrpcDataServiceConfig {
    pub fn get_listen_address(&self) -> String {
        if let Some(config) = &self.data_service_grpc_non_tls_config {
            config.data_service_grpc_listen_address.clone()
        } else if let Some(config) = &self.data_service_grpc_tls_config {
            config.data_service_grpc_listen_address.clone()
        } else {
            panic!("No grpc config provided");
        }
    }
}
