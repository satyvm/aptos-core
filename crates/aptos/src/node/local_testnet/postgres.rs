...
impl PostgresManager {
    ...
    async fn recreate_host_database(&self) -> Result<()> {
        ...
        // Spawn the connection object in its own task. This is what does the actual
        ...
        client
            .query(
                &format!("DROP DATABASE IF EXISTS {}", self.args.postgres_database),
                &[],
            )
            .await
        ...
        client
            .query(
                &format!("CREATE DATABASE {}", self.args.postgres_database),
                &[],
            )
            .await
            .context("Failed to drop database")?;
