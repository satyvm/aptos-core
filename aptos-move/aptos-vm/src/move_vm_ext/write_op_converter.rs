    ...
    pub(crate) fn convert_resource_group_v1(
        ...
        let post_group_size = group_changes.into_resources().into_iter().try_fold(
            ...
            |cur_size, (tag, current_op)| {
                let tag_size = bcs::serialized_size(&tag).map_err(tag_serialization_error)? as u64;
...
mod tests {
    ...
    fn size_computation_delete_modify_ops() {
        ...
        let change_set = AccountChangeSet::from_modules_resources(
            BTreeMap::new(),
            BTreeMap::from([
        ...
        let group_write = converter
            .convert_resource_group_v1(&key, change_set, false)
            .unwrap();
    ...
    fn size_computation_new_op() {
        ...
        let change_set = AccountChangeSet::from_modules_resources(
            BTreeMap::new(),
            BTreeMap::from([(mock_tag_2(), MoveStorageOp::New(vec![3, 3, 3].into()))]),
        );
    ...
    fn size_computation_new_group() {
        ...
        let change_set = AccountChangeSet::from_modules_resources(
            BTreeMap::new(),
            BTreeMap::from([(mock_tag_1(), MoveStorageOp::New(vec![2, 2].into()))]),
        );
        let key = StateKey::raw(vec![0]);
        let converter = WriteOpConverter::new(&resolver, true);
        let group_write = converter
